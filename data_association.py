...
def iou(bb_test,bb_gt): 
  ...
  box1_area = abs( (bb_test[2]-bb_test[0]) * (bb_test[3]-bb_test[1]) )
  box2_area = abs( (bb_gt[2]-bb_gt[0]) * (bb_gt[3]-bb_gt[1]) )
...
def associate_detections_to_trackers(detections,trackers,iou_threshold = 0.3):
  ...
  for d,det in enumerate(detections):
    for t,trk in enumerate(trackers):
      iou_matrix[d,t] = iou(det,trk) # det: detected boxes, trk: predicted boxes using KF?
  ...
  for m in matched_indices:
    # if iou_matrix[m[1],m[0]] < iou_threshold:
    # print(m)
